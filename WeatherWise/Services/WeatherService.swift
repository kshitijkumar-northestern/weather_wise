...
class WeatherService: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    // Production interval: 30 minutes
    ...
    override init() {
        ...
        if apiKey.isEmpty {
            DispatchQueue.main.async {
                self.errorMessage = "API key is not configured. Copy Secrets.example.plist to Secrets.plist and add your OpenWeatherMap API key."
    ...
    private func sendWeatherNotification(_ weather: WeatherModel) {
        ...
        if weather.isGoodWeather {
            print("Good weather detected! Sending notification")
